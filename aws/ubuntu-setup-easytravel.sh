...
installDynatrace() {
  if [ -n "${DT_TENANT_URL}" ]; then
	...
	echo wget -nv -O oneagent.sh "$DT_TENANT/api/v1/deployment/installer/agent/unix/default/latest?Api-Token=$DT_PAAS_TOKEN&arch=x86&flavor=default"
	wget -nv -O oneagent.sh "$DT_TENANT/api/v1/deployment/installer/agent/unix/default/latest?Api-Token=$DT_PAAS_TOKEN&arch=x86&flavor=default"
    sh oneagent.sh
  ##--set-app-log-content-access=true --set-system-logs-access-enabled=true --set-infra-only=false --set-host-name=easytravel.staging
  fi
}
...
=======
  ...
  sed -i 's/config.reUseChromeDriverFrequency=4/config.reUseChromeDriverFrequency=3/g' /home/$USER/easytravel-2.0.0-x64/resources/easyTravelConfig.propertieseasyTravelConfig.properties
